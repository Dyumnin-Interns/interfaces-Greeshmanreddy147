...
class TB:
    def __init__(self, name, entity, log):
        ...
        self.stats = []
        self.ref_address = {
            'A_status': 0, 'B_status': 1, 'Y_status': 2,
    ...
    def cvr(self):
        ...
        self.p.addConstraint(
            lambda rd_en, wd_en, rd_rdy: rd_en == 1 if wd_en == 0 and rd_rdy == 1 else rd_en == 0,
            ['read_en', 'write_en', 'read_rdy']
        )
        self.p.addConstraint(
            lambda rd_en, wd_en, wd_rdy: wd_en == 1 if rd_en == 0 and wd_rdy == 1 else wd_en == 0,
...
async def dut_test(dut):
    ...
    await tbh.writer._driver_send({'addr': 4, 'val': 0})
    await tbh.writer._driver_send({'addr': 5, 'val': 0})
    sample_fnc(0, 0)
    await tbh.reader._driver_send({'addr': 3, 'val': 0})
    log.debug(f"[functional] a:0 b:0 y:{dut.read_data.value.integer}")

    await tbh.writer._driver_send({'addr': 4, 'val': 0})
    await tbh.writer._driver_send({'addr': 5, 'val': 1})
    sample_fnc(0, 1)
    await tbh.reader._driver_send({'addr': 3, 'val': 0})
    log.debug(f"[functional] a:0 b:1 y:{dut.read_data.value.integer}")

    await tbh.writer._driver_send({'addr': 4, 'val': 1})
    await tbh.writer._driver_send({'addr': 5, 'val': 0})
    sample_fnc(1, 0)
    await tbh.reader._driver_send({'addr': 3, 'val': 0})
    ...
    for i in range(100):
        ...
        if x.get('read_en') == 1:
            await tbh.reader._driver_send({'addr': x.get('read_address'), 'val': 0})
